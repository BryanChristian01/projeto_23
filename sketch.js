
const Engine = Matter.Engine;
const World = Matter.World;
const Bodies = Matter.Bodies;
const Body = Matter.Body;
var block1,block2;
var plane;
var rotator1,rotator2,rotator3;
var angle1=60,angle2=60,angle3=60;
var rotator_options;
var particle1,particle2,particle3,particle4,particle5;
var engine,world;

function preload(){
	
}

function setup() {
	createCanvas(800, 600);
engine = Engine.create();
	world = engine.world;

	//Crie os Corpos aqui.
var plane_options = {
	isStatic: true
}
plane = Bodies.rectangle(400,550,800,20,plane_options);
World.add(world,plane);
block1 = Bodies.rectangle(200,380,150,20,plane_options);
World.add(world,block1);
block2 = Bodies.rectangle(450,380,150,20,plane_options);
World.add(world,block2);

var particle_options = {
	restitution:0.4,
	friction:0.02
}

particle1 = Bodies.circle(220,10,30,particle_options)
World.add(world,particle1);

particle2 = Bodies.circle(225,10,30,particle_options)
World.add(world,particle2);

particle3 = Bodies.circle(230,10,30,particle_options)
World.add(world,particle3);

particle4 = Bodies.circle(235,10,30,particle_options)
World.add(world,particle4);

particle5 = Bodies.circle(240,10,30,particle_options)
World.add(world,particle5);


var rotator_options = {isStatic:true};
rotator1 = Bodies.rectangle(315,200,150,20,rotator_options);
World.add(world,rotator1);
rotator2 = Bodies.rectangle(315,200,150,20,rotator_options);
World.add(world,rotator2);
rotator3 = Bodies.rectangle(315,200,150,20,rotator_options);
World.add(world,rotator3);



Engine.update(engine);
  
}


function draw() {
  
	background("green");
	Engine.update(engine);
  rectMode(CENTER);

fill("darkred")

rect(block1.position.x,block1.position.y,150,20);
rect(block2.position.x,block2.position.y,150,20);
rect(plane.position.x,plane.position.y,800,20);
// rect(rotator1.position.x,rotator1.position.y,150,20);
// rect(rotator2.position.x,rotator2.position.y,150,20);
// rect(rotator3.position.x,rotator3.position.y,150,20);
ellipse(particle1.position.x,particle1.position.y,30)
ellipse(particle2.position.x,particle2.position.y,30) 
ellipse(particle3.position.x,particle3.position.y,30)
ellipse(particle4.position.x,particle4.position.y,30)
ellipse(particle5.position.x,particle5.position.y,30)

Matter.Body.rotate(rotator1,angle1)
push()
translate(rotator1.position.x,rotator1.position.y);
rotate(angle1);
rect(0,0,150,20);
pop()
angle1 +=3;

Matter.Body.rotate(rotator2,angle2)
push()
translate(rotator2.position.x,rotator2.position.y);
rotate(angle2);
rect(0,0,150,20);
pop()
angle2 +=4;

Matter.Body.rotate(rotator3,angle3)
push()
translate(rotator3.position.x,rotator3.position.y);
rotate(angle3);
rect(0,0,150,20);
pop()
angle3 +=5;

drawSprites();
 
}



